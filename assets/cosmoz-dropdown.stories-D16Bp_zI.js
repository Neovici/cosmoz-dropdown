...
`,ro=t=>{const{placement:e,strategy:n,middleware:o,render:r}=t,{active:s,onToggle:i}=eo(t),c=$t(),{styles:l,setReference:a,setFloating:u}=Je({placement:e,strategy:n,middleware:o}),d=Q(f=>{c.current=f,u(f)},[u]);return rt(()=>{const f=c.current;f&&(s&&!f.matches(":popover-open")&&f.showPopover?.(),!s&&f.matches(":popover-open")&&f.hidePopover?.())},[s]),B`
		...
		>
	`};customElements.define("cosmoz-dropdown",st(ro,{styleSheets:[no]}));const so=vt`
	:host {
		display: contents;
